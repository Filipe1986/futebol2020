...
dates = open('erros.txt', 'r')


for i in dates:
    
    dateFormatted = i.strip()
    params['dt_pesquisa'] = dateFormatted
    time.sleep(1)

    FILE_NAME = 'certos1.csv'
    FILE_LOG  = 'erros1-log.csv'
    ...
    if(r.status_code == 200 and len(r.content) > 10):
        ...
        if(int(htmlNum[0]) == int(checkSize)):
            for i in htmlf:
                nome = ''
                if(len(i.xpath('.//div[@class="modal-header"]/h4/text()'))):
                    nome = i.xpath('.//div[@class="modal-header"]/h4/text()')[0]
